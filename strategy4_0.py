"""
based on holding qty of opposite asset for each asset unsold at a higher price
"""
...
class Strategy:
    ...
    def __get_signal1_a(self):
        if self.priceatri.yes_executed_avgpri >= self.priceatri.yes_executed_wap:
            return True
        else:
            return False

    def __get_signal1_b(self):
        if self.priceatri.no_executed_avgpri <= self.priceatri.yes_executed_wap:
            return True
        else:
            return False

    def __get_signal2(self):
        if self.priceatri.yes_value_per_pp > self.priceatri.no_value_per_pp:
            return True
        else:
            return False

    def _get_situation(self):
        if self.__get_signal2():
            self.strong_side = "Yes"
            if self.__get_signal1_a() == self.__get_signal1_b() == True:
                self.signal_strength = "Strong"
            elif self.__get_signal1_a() != self.__get_signal1_b():
                self.signal_strength = "Weak"
        elif not self.__get_signal2():
            self.strong_side = "No"
            if self.__get_signal1_a() == self.__get_signal1_b() == False:
                self.signal_strength = "Strong"
            elif self.__get_signal1_a() != self.__get_signal1_b():
                self.signal_strength = "Weak"
    ...
    def _set_qty(self):
        ...
        if (self.strong_side == "Yes") \
                and (self.signal_strength == "Strong") \
                and (self.my_buy_price_nos > self.priceatri.no_best_price + self.opp_direction_move_margin):
